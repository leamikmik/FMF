...
3. Substitucija => $t=x; dt=dx$
4. Per partes => $\int u*dv=u*v-\int v*du$
5. $\int \frac{P(x)}{Q(X)}dx=\int K(x)dx + \int \frac{R(x)}{Q(x)}dx$
...
6. Univerzalna trigonometrična substitucija: 
...
7. $\int \frac{f'(x)}{f(x)}dx=\ln|f(x)|+c$
---
# Določeni integrali
